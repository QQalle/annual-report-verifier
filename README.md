...
- `/analyze` — drag one newer and one prior report into the two viewers. The app detects the years, enforces adjacency, compares the reports, and highlights only the current-year column in the newer report in yellow. Hovering or selecting a compared figure also reveals its row key and year; previous/next controls step through red-only or red-and-gray findings.
